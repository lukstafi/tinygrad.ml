...
- **Known issue**: Matmul backward produces incorrect gradients for weight matrices when `batch > 1` and inputs are non-identity. Root cause is a subtle interaction between EXPAND backward (batch reduction) and the 3D index transformations in the generated kernel. Forward matmul and matmul backward w.r.t. input (x) work correctly. This will be addressed in a future round.
