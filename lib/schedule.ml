...
(** Lower a reduction operation to a kernel.
    For REDUCE_AXIS(expr, ADD/MAX, axes), generates:
      out[0] = identity; for(i) out[0] = reduce_op(out[0], expr[i]);
...
let lower_reduce_kernel ~device ~input_numel ~output_numel ~kname
  ...
  | None ->
    ...
    (* For full reduction to a single value: init out[0], loop, accumulate *)
    if output_numel = 1 then begin
      ...
      (* Partial reduction not yet supported — fall back *)
      None
...
let create_schedule ?(device="CPU") ~numel (roots : Uop.t list) : exec_item list =
  ...
  List.iter (fun (root : Uop.t) ->
    ...
    (* Step 2: Schedule any unrealized inner REDUCE_AXIS nodes first.
       This handles mean() = sum() * (1/n) where sum is a nested REDUCE_AXIS. *)
    List.iter (fun (u : Uop.t) ->
      if u.op = Ops.REDUCE_AXIS && get_realized u.id = None then begin
        let reduce_op, source_uop = match u.arg with
